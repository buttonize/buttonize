...
export interface ButtonizeAppProps {
	/**
	 * Name of the app. Users can search for the app by its name.
	 *
	 ...
	 * to only grant groups of users access to apps with certain tags.
	 *
	 * @default [] Empty array
	 ...
	 * @example Tool to give support team the ability to generate special discount codes for demanding customers.
	 */
	description?: string
	 ...
	 * In most cases, you should use `Buttonize.init` instead.
	 *
	...
	/**
	 * **Use this approach only if you really need to.**
	 * In most, cases you should not need to use this.
	 *
	 * Custom execution role used for invoking AWS actions.
	 *
	 * @default Buttonize creates its own role with all the necessary permissions. The principle of least privilege is obeyed.
	 */
...
export interface ButtonizeAppPageProps {
	 ...
	 * For example, it can be used to pre-fetch data for `Input.select` options
	 * or simply to display text with `Display.text`, etc.
	 *
	 * @default {} No actions
	 * @example
	 *
	...
	/**
	 * Title of the page. To give users context about what they can do on the page.
	 *
...
/**
 * The Buttonize App which can be deployed and used at {@link https://app.buttonize.io}
 *
 * @example
 *
 * ```ts
...
export class ButtonizeApp extends Construct {
	...
	/**
	 * Execution role generated by Buttonize to be used for invoking the actions.
	 *
	 ...
	 *
	 * @param id Unique ID of the page in the scope of the ButtonizeApp.
	 * @param props Page props such as `body` to define the contents of the page.
	 */
