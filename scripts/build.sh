# Stores current git commit hash to version.ts file as a metadata sent to Buttonize IaC handlers
echo "// Do not edit this file. It's modified in build process\nexport const version = '$(git rev-parse HEAD)'" > cdk/custom-resources/version.ts && \
